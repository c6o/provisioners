...
export const provisionMixin = (base: baseProvisionerType) => class extends base {
    ...
    async connectMongoDbClient(processor: Processor, attempt) {
        ...
        this.manager.status?.log(`Attempt ${attempt + 1} to connect to mongo on local port ${processor.lastResult.other.localPort}`)
    ...
    async provisionMongoDb(processor) {
        this.configMap = {}
        await asyncForEach(this.spec.config, async dbConfig => await this.setupDb(dbConfig))
    ...
    async setupDb(dbConfig) {
        try {
            ...
            if (process.env.TRAXITT_ENV == 'development') {
                this.manager.status?.log(`Connection string ${connectionString}`)
            }
