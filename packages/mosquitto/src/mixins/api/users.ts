...
export const userMgmtMixin = (base: baseProvisionerType) => class extends base {
    ...
    async addUser(username: string, password: string, namespace: string, restart: boolean) {
        ...
        if (!found) {
            ...
            const updatedResult = await super.cluster.put(settings.manifest, settings.configmap)
            ...
            if (restart) {
                ...
                await DeploymentHelper.from(namespace, this.deploymentName).restartDeployment(super.cluster)
            }
        }
